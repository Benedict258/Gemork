...
## 0. Foundation Building Blocks (Reused Open Source)

Gemork is not being built from a blank slate. Three open-source projects are cloned locally under a `BuildingBlocks/` directory and serve as the starting point for specific subsystems — refactored and reworked into Gemork's own architecture, not used as-is.

| Repo | Role in Gemork | Subsystem it seeds |
|---|---|---|
| **Agent Zero** | Core agent-loop reference: plan → tool use → subordinate agent delegation → self-correction | Task/Plan Engine + Sub-agent Coordinator (Architecture §3) |
| **WebBrain** | Ask/Act mode pattern (read-only vs. action-capable browser control) | Browser Extension + directly informs the Tier 1/Tier 3 guardrail split (Architecture §8) |
| **IRIS GO** | Reference architecture for desktop ↔ cloud ↔ mobile relay and remote task control | Cloud Bridge + Mobile relay (Architecture §1 diagram, §7) |

**Ground rules for use:**
- Each repo is cloned into `BuildingBlocks/<repo-name>/` and treated as a reference/seed, not a dependency — code is extracted, refactored, and rewritten to fit Gemork's own module boundaries, naming, and guardrail model.
- Docker/sandbox requirements in the source repos (e.g. Agent Zero's Dockerized runtime) are stripped in favor of Gemork's installed-app, non-sandboxed execution model.
- License terms for each repo must be confirmed compatible with Gemork's intended post-hackathon commercial/product path before any code is merged into the main Gemork codebase (not just referenced in `BuildingBlocks/`).
- Nothing from `BuildingBlocks/` ships directly in the final build — it exists to accelerate design decisions and provide working reference implementations of hard problems (sub-agent spawning, ask/act browser control, remote relay) that would otherwise be built from scratch under hackathon time pressure.
